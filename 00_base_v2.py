...
class MainPage:
    ...
    def __init__(self):
        ...
        # Set up GUI Frame
        self.main_frame = Frame(padx=50, pady=50)
        ...
        self.main_subheading.grid(row=1, pady=20)
...
class Quiz:
    ...
    def __init__(self, how_many):
        ...
        self.all_fears = list(csv.reader(file, delimiter=","))
        file.close()
        ...
        self.blank_row = Label(self.text_frame, text="")
        self.blank_row.grid(row=1, column=0, pady=10)
        ...
        self.quiz_frame.grid(row=4, column=0, padx=50)
        ...
        for item in range(0, 4):
            self.choice_buttons = Button(self.quiz_frame, bg="#80c5ff", font=button_font, width=18, height=3, command=lambda i=item: self.check_answer(self.button_fear_list[i], i))

            self.choice_buttons_ref.append(self.choice_buttons)

            self.choice_buttons.grid(row=item // 2, column=item % 2, pady=15, padx=15)

        # create a label to tell the user if they got it correct or incorrect
        self.correct_incorrect_label = Label(self.quiz_frame, text="", font=("Arial", "18", "bold"))
        
        self.correct_incorrect_label.grid(row=5, columnspan=2, pady=10)

        # adds a blank row to place 'next button' at bottom of window
        self.blank = Label(self.quiz_frame, text="")

        self.blank.grid(row=6, column=1, pady=10)

        # add next button
        self.next_button = Button(self.quiz_frame, text="Next Question",
                            bg="#D5E8D4", font=button_font, width=18, height=2, command=self.new_question)
    ...
    def get_fears(self):
        ...
        while len(self.question_fear_list) < 4:
            ...
            fears = random.choice(self.all_fears)
            if fears not in fear_list:

                self.question_fear_list.append(fears)
                fear_list.append(fears)
    ...
    def new_question(self):
        ...
        self.correct_incorrect_label.config(text="")
        ...
        # remove correct fear from master list
        self.all_fears.remove(self.correct_fear)
    ...
    def check_answer(self, user_answer, button_pressed):
        ...
        if user_answer[1] == correct_answer[1]:
            ...
            # configures the correct / incorrect label (correct)
            self.correct_incorrect_label.config(text="You Got it Correct!", fg="#40bd40")

        # if user selects incorrect answer, add one to the incorrect counter
        else:
            self.incorrect_answers += 1
            self.choice_buttons_ref[button_pressed].config(bg="#e85d5d")

            # configures the correct / incorrect label (incorrect)
            self.correct_incorrect_label.config(text="You Got it Incorrect :(", fg="#e85d5d")

        # set the correct answer to green
        self.choice_buttons_ref[self.button_number].config(bg="#40bd40")
...
class DisplayStats:
    ...
    def __init__(self, correct_answers, incorrect_answers, how_many, stats_button):
        ...
        # create dismiss button, upon press send to dismiss function
    ...
    # function to dismiss the stats window upon button press
